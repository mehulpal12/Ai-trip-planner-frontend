...
import {
  ...
  LogOut, Bell, Search, Map, Home, Bot, User, Menu, X, Grid, List, 
  ChevronLeft, ChevronRight, SlidersHorizontal, Calendar, MapPin, 
  DollarSign, FileText, Trash2, Edit3
} from "lucide-react";
...
const getDestinationLabel = (trip: ApiTrip) => {
  ...
  const locationParts = [destination.city, destination.state, destination.country]
...
const normalizeTrip = (trip: ApiTrip, currentUserName = "You"): TripItem => ({
  ...
  members: (trip.members ?? []).map((member) => ({
    id: member.id,
    name: member.memberName || (member.role === "OWNER" ? currentUserName : member.userId),
  })),
  activities: [
    {
      id: `created-${trip.id}`,
...
const INITIAL_TRIPS: TripItem[] = [
  {
    id: "trip-1",
    title: "Alpine Minimalism Expedition",
    destination: "Icelandic Highlands",
    startDate: "2026-06-02",
    endDate: "2026-06-12",
    budget: 4500,
    notes: "Focus on capturing architectural frames matching the volcanic landscape structures.",
    owner: { name: "Pro Explorer", email: "traveler@aether.io", avatar: "https://lh3.googleusercontent.com/aida-public/AB6AXuCf85sTSU0A5UOsFd-Dprjjfjedp4nkjSOxDj72qX6Rk8MNdqetl4mhhA1UldyRads1O-Ud9-FCqqqQObqFGxRXXpnujJWF77qqyYisF4d-6UbRfzzvv-WrhHTR6IA_3rxzhf3pxQP5K0_utNtu43KRIyJlLdzhhxeUDiw5gaXCf7kLwXiYBl--itBL-wzQz_AvzIhJhacMaDTHk5BEPEYNnjQQOkRcyWfOLKiNfbQFY1wlCxUFSxbJgZaqmYjdh94S4pzqYNzPJA" },
    members: [
      { name: "Sarah K.", avatar: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100" },
      { name: "Alex M.", avatar: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100" }
    ],
    activities: [
      { id: "a1", text: "Booked ice cave photography permit", time: "2 hours ago" },
      { id: "a2", text: "Route updated to avoid Southern ridge closures", time: "Yesterday" }
    ]
  },
  {
    id: "trip-2",
    title: "Nordic Architecture Tour",
    destination: "Oslo & Copenhagen",
    startDate: "2026-08-15",
    endDate: "2026-08-22",
    budget: 3200,
    notes: "Exploring modular housing concepts and public waterfront construction design.",
    owner: { name: "Pro Explorer", email: "traveler@aether.io", avatar: "https://lh3.googleusercontent.com/aida-public/AB6AXuCf85sTSU0A5UOsFd-Dprjjfjedp4nkjSOxDj72qX6Rk8MNdqetl4mhhA1UldyRads1O-Ud9-FCqqqQObqFGxRXXpnujJWF77qqyYisF4d-6UbRfzzvv-WrhHTR6IA_3rxzhf3pxQP5K0_utNtu43KRIyJlLdzhhxeUDiw5gaXCf7kLwXiYBl--itBL-wzQz_AvzIhJhacMaDTHk5BEPEYNnjQQOkRcyWfOLKiNfbQFY1wlCxUFSxbJgZaqmYjdh94S4pzqYNzPJA" },
    members: [],
    activities: [
      { id: "a3", text: "Trip structure initialized", time: "3 days ago" }
    ]
  }
];
...
const TripsPage: React.FC<TripsPageProps> = ({
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingTrip) {
        ...
        setTrips((current) => current.map((trip) => trip.id === editingTrip.id ? updatedTrip : trip));
        ...
        setTrips((current) => [newTrip, ...current.filter((trip) => trip.id !== newTrip.id)]);
        triggerToast(`"${formState.title}" successfully organized!`);
      }
      setIsModalOpen(false);
      await fetchTrips();
    } catch (error) {
      triggerToast(error instanceof Error ? error.message : "Trip save failed.");
    }
  };
  ...
  const handleDeleteTrip = async (id: string, e?: React.MouseEvent) => {
    ...
    try {
      ...
      triggerToast(error instanceof Error ? error.message : "Trip deletion failed.");
    }
  };
      ...
      {!selectedTrip ? (
        <>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/[0.06] pb-5">
            ...
            <button onClick={openCreateModal} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-cyan-500 text-[#080a10] text-sm font-semibold hover:bg-cyan-400 transition-all duration-200 shadow-[0_0_20px_rgba(0,209,255,0.2)] cursor-pointer border-none">
          ...
          {isTripsLoading ? (
            ...
            <div className="text-center py-12 border border-rose-500/20 bg-rose-500/[0.03] rounded-2xl">
              ...
              <button onClick={fetchTrips} className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-xs text-slate-200 hover:bg-white/10 transition-colors">
                Retry
              </button>
            </div>
          ) : paginatedTrips.length === 0 ? (
            <div className="text-center py-16 border border-dashed border-white/[0.08] rounded-2xl">
              <p className="text-slate-400 text-sm">No matching travel blueprints discovered.</p>
            </div>
          ) : viewMode === "grid" ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {paginatedTrips.map((trip) => (
                <div key={trip.id} onClick={() => setSelectedTrip(trip)} className="group relative border border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.04] p-5 rounded-2xl transition-all duration-300 hover:-translate-y-0.5 backdrop-blur-xl cursor-pointer flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start gap-4 mb-3">
                      ...
                      <div className="flex items-center gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                        <button onClick={(e) => openEditModal(trip, e)} className="p-1 text-slate-400 hover:text-cyan-400 bg-transparent border-none cursor-pointer"><Edit3 size={14} /></button>
                        <button onClick={(e) => { e.stopPropagation(); setShowDeleteConfirm(trip.id); }} className="p-1 text-slate-400 hover:text-rose-400 bg-transparent border-none cursor-pointer"><Trash2 size={14} /></button>
                      </div>
                    </div>
                  ...
                  <div className="border-t border-white/[0.06] mt-5 pt-4 flex justify-between items-center text-xs">
                    <div className="text-slate-400 flex items-center gap-1"><Calendar size={12} /> {trip.startDate}</div>
            ...
            <div className="space-y-3">
              {paginatedTrips.map((trip) => (
                <div key={trip.id} onClick={() => setSelectedTrip(trip)} className="group border border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.04] px-5 py-4 rounded-xl transition-all flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 cursor-pointer backdrop-blur-xl">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-xs text-slate-400">
                      ...
                      <span className="flex items-center gap-1"><Calendar size={12} /> {trip.startDate} to {trip.endDate}</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-6 border-t sm:border-none border-white/[0.06] pt-3 sm:pt-0">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={(e) => openEditModal(trip, e)} className="p-1.5 text-slate-400 hover:text-cyan-400 bg-white/5 rounded-lg border-none cursor-pointer"><Edit3 size={13} /></button>
                      <button onClick={(e) => { e.stopPropagation(); setShowDeleteConfirm(trip.id); }} className="p-1.5 text-slate-400 hover:text-rose-400 bg-white/5 rounded-lg border-none cursor-pointer"><Trash2 size={13} /></button>
                    </div>
                  </div>
                </div>
              ))}
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 pt-4">
              <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="p-2 rounded-xl bg-white/5 border border-white/10 text-slate-400 disabled:opacity-30 disabled:cursor-not-allowed hover:text-white transition-colors cursor-pointer"><ChevronLeft size={16} /></button>
              <span className="text-xs text-slate-400 font-medium px-3">Page {currentPage} of {totalPages}</span>
              <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="p-2 rounded-xl bg-white/5 border border-white/10 text-slate-400 disabled:opacity-30 disabled:cursor-not-allowed hover:text-white transition-colors cursor-pointer"><ChevronRight size={16} /></button>
            </div>
          )}
        </>
      ) : (
              ...
              <div className="border border-white/[0.07] bg-white/[0.03] backdrop-blur-xl p-6 rounded-2xl relative overflow-hidden">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setShowDeleteConfirm(selectedTrip.id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-rose-500/10 border border-rose-500/20 text-xs font-medium hover:bg-rose-500/20 text-rose-400 transition-all cursor-pointer"><Trash2 size={13} /> Delete</button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 border-y border-white/[0.06] py-4 my-4 text-sm">
                  <div>
                    ...
                    <span className="text-slate-200 font-medium flex items-center gap-1.5 mt-0.5"><Calendar size={14} className="text-slate-400" /> {selectedTrip.startDate}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 text-xs block">End Pipeline</span>
                    <span className="text-slate-200 font-medium flex items-center gap-1.5 mt-0.5"><Calendar size={14} className="text-slate-400" /> {selectedTrip.endDate}</span>
                  </div>
                  <div>
            ...
            <div className="xl:col-span-4 space-y-6">
              <div className="border border-white/[0.07] bg-white/[0.03] backdrop-blur-xl p-5 rounded-2xl">
                ...
                <div className="flex items-center gap-3 bg-[#080a10]/30 p-3 rounded-xl border border-white/[0.04]">
                  <div className="h-9 w-9 rounded-full bg-gradient-to-tr from-cyan-500 to-sky-600 flex items-center justify-center font-bold text-xs text-white">PE</div>
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-white truncate">{selectedTrip.owner.name}</p>
                    <p className="text-xs text-slate-500 truncate">{selectedTrip.owner.email}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="border border-white/[0.07] bg-white/[0.03] backdrop-blur-xl p-5 rounded-2xl">
                ...
                {selectedTrip.members.length === 0 ? (
                  <p className="text-xs text-slate-500 italic py-2">No active workspace nodes joined yet.</p>
                ) : (
                  <div className="space-y-2.5">
                    {selectedTrip.members.map((m, idx) => (
                      <div key={m.id ?? idx} className="flex items-center gap-2.5 text-sm text-slate-300">
                        {m.avatar ? (
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="relative bg-[#0c0f17] border border-white/[0.08] w-full max-w-lg p-6 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] z-10 max-h-[90vh] overflow-y-auto text-left">
              <button onClick={() => setIsModalOpen(false)} className="absolute top-4 right-4 text-slate-400 hover:text-white transition-colors bg-transparent border-none cursor-pointer"><X size={18} /></button>
              ...
              <form onSubmit={handleFormSubmit} className="space-y-4 text-sm">
                ...
                <div className="flex justify-end gap-3 pt-2">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 rounded-xl border border-white/10 text-slate-400 hover:text-white transition-colors bg-transparent cursor-pointer">Cancel</button>
                  <button type="submit" className="px-5 py-2 rounded-xl bg-cyan-50 text-[#080a10] font-semibold hover:bg-cyan-400 transition-colors shadow-lg border-none cursor-pointer">Save Configuration</button>
                </div>
              </form>
            </motion.div>
          </div>
        ...
        {showDeleteConfirm && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-[#0c0f17] border border-rose-500/20 w-full max-w-sm p-5 rounded-2xl shadow-2xl z-10 text-center">
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowDeleteConfirm(null)} className="flex-1 py-2 rounded-xl border border-white/10 text-xs text-slate-400 hover:text-white transition-colors bg-transparent cursor-pointer">Abort</button>
                <button onClick={() => handleDeleteTrip(showDeleteConfirm)} className="flex-1 py-2 rounded-xl bg-rose-500 text-white text-xs font-semibold hover:bg-rose-400 transition-colors border-none cursor-pointer">Confirm Removal</button>
              </div>
            </motion.div>
          </div>
        )}
...
const AetherDashboard: React.FC = () => {
  ...
  const userDisplayName = isLoading ? "Loading..." : userData?.name || userData?.email || "Traveler";
  ...
  const fetchTrips = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch trips. Server responded with status ${response.status}`);
      }

      const result = await response.json();
      const payload = Array.isArray(result) ? result : result.data ?? result.trips ?? [];
      setTrips(payload.map((trip: ApiTrip) => normalizeTrip(trip, userDisplayName)));
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (heroRef.current) {
        tl.fromTo(heroRef.current, { opacity: 0, y: 25 }, { opacity: 1, y: 0, duration: 0.8, ease: "power3.out" });
      }
  ...
  return (
    <div className="relative min-h-screen bg-[#080a10] text-white overflow-x-hidden font-sans antialiased selection:bg-cyan-500/30">
      ...
      <AnimatePresence>
        {toast && (
          <motion.div initial={{ opacity: 0, y: -20, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.9 }} className="fixed top-6 right-6 z-[130] bg-cyan-900/80 backdrop-blur-xl border border-cyan-500/30 text-cyan-200 px-5 py-3 rounded-xl shadow-[0_0_30px_rgba(0,209,255,0.15)] flex items-center gap-2 text-sm font-medium">
      ...
      <header className="fixed top-0 inset-x-0 z-50 h-[68px] flex items-center justify-between px-6 md:px-12 border-b border-white/[0.06] bg-[#080a10]/80 backdrop-blur-2xl">
        <div className="flex items-center gap-10">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-bold tracking-tight text-white">Aether<span className="text-cyan-400">.</span></span>
          </div>
          <nav className="hidden md:flex items-center gap-1">
            {["Dashboard", "Destinations", "Itineraries", "AI Assistant", "Community"].map((item) => (
              <button 
                key={item} 
                onClick={() => setActiveTab(item === "AI Assistant" ? "AI Planner" : item === "Destinations" || item === "Itineraries" ? "My Trips" : item)}
        ...
        <div className="flex items-center gap-3">
          <button className="p-2 rounded-xl text-slate-400 hover:text-slate-200 hover:bg-white/[0.06] transition-all bg-transparent border-none cursor-pointer"><Search size={18} /></button>
          ...
          >
            <Plus size={15} />Start Planning
          </button>
          <div className="relative h-9 w-9 rounded-full overflow-hidden border-2 border-cyan-500/30">
            <Image src="https://lh3.googleusercontent.com/aida-public/AB6AXuCf85sTSU0A5UOsFd-Dprjjfjedp4nkjSOxDj72qX6Rk8MNdqetl4mhhA1UldyRads1O-Ud9-FCqqqQObqFGxRXXpnujJWF77qqyYisF4d-6UbRfzzvv-WrhHTR6IA_3rxzhf3pxQP5K0_utNtu43KRIyJlLdzhhxeUDiw5gaXCf7kLwXiYBl--itBL-wzQz_AvzIhJhacMaDTHk5BEPEYNnjQQOkRcyWfOLKiNfbQFY1wlCxUFSxbJgZaqmYjdh94S4pzqYNzPJA" alt={userDisplayName} fill className="object-cover" sizes="36px" />
          </div>
          <button className="md:hidden p-2 text-slate-400 hover:text-slate-200 bg-transparent border-none cursor-pointer" onClick={() => setSidebarOpen(!sidebarOpen)}>{sidebarOpen ? <X size={20} /> : <Menu size={20} />}</button>
        </div>
      </header>

      <div className="flex min-h-screen pt-[68px]">
        {/* ── Adaptive Layout Drawer Sidebar ── */}
        <aside className={[
          ...
          <div className="mb-6 px-2">
            <p className="text-base font-bold text-white truncate">{userDisplayName}</p>
            <p className="text-xs text-slate-500 mt-0.5 font-medium">Pro Explorer</p>
          </div>
          ...
          <div className="border-t border-white/[0.06] pt-4 space-y-1">
            <SideNavItem icon={<HelpCircle size={17} />} label="Support" onClick={() => setActiveTab("Settings")} />
            <SideNavItem icon={<LogOut size={17} />} label="Logout" danger onClick={() => router.push("/")} />
          </div>
        </aside>
      ...
      <footer className="relative z-10 md:ml-64 border-t border-white/[0.06] bg-[#080a10]/80 backdrop-blur-xl px-6 md:px-12 py-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <div className="h-6 w-6 rounded-lg bg-gradient-to-br from-cyan-400 to-sky-600 flex items-center justify-center"><Map size={12} className="text-white" /></div>
            <span className="text-sm font-bold text-white">Aether Travel</span>
          </div>
          <p className="text-slate-600 text-xs">© 2026 Aether Travel Technologies. All rights reserved.</p>
        </div>
        <div className="flex flex-wrap gap-6">
          {["Privacy Policy", "Terms of Service", "Cookie Settings", "Global Support"].map((link) => (
            <a key={link} href="#" onClick={(e) => { e.preventDefault(); setActiveTab("Settings"); }} className="text-slate-500 text-xs hover:text-cyan-400 transition-colors duration-200 no-underline">{link}</a>
          ))}
        </div>
      </footer>
    </div>
